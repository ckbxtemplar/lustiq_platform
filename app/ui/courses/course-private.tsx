...
export default function CoursePrivate({ data }: { data: any }) {
						...
						<aside className="sidebar">
							...
							<div className="course_info_card d-none d-lg-block position-absolute me-5">
								...
								<ul className="course_info_list unordered_list_block">
									<li>
										<span><i className="fas fa-user"></i> {t('author')}</span>
										<strong>{data?.author?.name || 'Unknown'}</strong>
									</li>
									<li className='d-none'>
										<span><i className="fas fa-chart-bar"></i> {t('level')}</span>
										<strong>{data?.Details?.level || 'Beginner'}</strong>
									</li>
									<li>
										<span><i className="fas fa-clock"></i> {t('duration')}</span>
										<strong>{data?.Details?.duration || '0'} {t('hours')}</strong>
									</li>
									<li>
										<span><i className="fas fa-video"></i> {t('lessons')}</span>
										<strong>{courseElements?.counter.video || '0'} {t('video')}</strong>
									</li>
								</ul>
							</div>
